...
void flashBuilder(const char *filein, const char *fileout, U8 mode)
{
	...
	if (size>0)
	{
		...
		switch (mode)
		{
			case 1:  {save(fileout,data,16,image128,image128_len);} break;
			case 2:  {save(fileout,data,32,image256,image256_len);} break;
			case 3:  {save(fileout,data,64,image512,image512_len);} break;
			default: {save(fileout,data,128,image1024,image1024_len);} break;
		};
	}
	else
	{
